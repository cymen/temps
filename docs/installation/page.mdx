...
Set `TEMPS_TRUST_LOOPBACK_FORWARDED_IP=true` when starting `temps serve` (or
`temps proxy` in a split installation) to use the visitor IP for analytics
and IP-based controls. The equivalent CLI flag is
`--trust-loopback-forwarded-ip`. Forwarded-IP trust is off by default.
