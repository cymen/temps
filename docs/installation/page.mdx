...
Start the proxy with `temps serve --trust-loopback-forwarded-ip` (or
`temps proxy --trust-loopback-forwarded-ip` in a split installation) to use
the visitor IP for analytics and IP-based controls. Forwarded-IP trust is off
by default. Enable it only when nginx connects to Temps over loopback and
